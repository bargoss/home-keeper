...
namespace Obi
{
    public class ObiCustomEmitter : ObiActor
    {
        ...
        public bool ParticleActive(int solverIndex)
        {
            return m_Solver.invMasses[solverIndex] > 0;
        }
        public bool EmitParticle(Vector3 position, Vector3 velocity, out int solverIndex)
        {
            ...
            solverIndex = solverIndices[index];

            m_Solver.startPositions[solverIndex] = m_Solver.positions[solverIndex] = position;
            m_Solver.velocities[solverIndex] = velocity;

            UpdateParticleMaterial(index);

            // now there's one active particle more:
            if (!ActivateParticle(index))
                return false;
        ...
        public void PushParticles(IEnumerable<ParticleInfo> particleInfos)
        {
            ...
            {
                EmitParticle(particleInfo.Position, particleInfo.Velocity, out _);
            }
        }
        public void PullParticles(List<ParticleInfo> particleInfos)
        {
            particleInfos.Clear();
            for (int i = 0; i < activeParticleCount; ++i)
            {
                ...
                {
                    Position = m_Solver.positions[solverIndex],
                    Velocity = m_Solver.velocities[solverIndex]
                });
            }
        }
